...
export default new Vuex.Store({
  ...
  getters: {
    ...
    random_movies_10(state) {
      return _.sampleSize(state.random_movies, 10);
    },
  },
      ...
      state.reviews = state.reviews.map((review) => {
        if (review.id == comment.review) {
          review.comments = review.comments.map((commentInReview) => {
            if (commentInReview.id === comment.id) {
              commentInReview.content = comment.content;
            }
  ...
  actions: {
      ...
      axios({
        ...
        .catch((err) => {
          if (
            ...
            err.response.data.non_field_errors?.include(
    ...
    uploadProfile(context, formData) {
      axios
        .post(`${DJANGO_URL}/api/v1/${context.state.username}/`, formData, {
          headers: {
            Authorization: `Token ${context.state.token}`,
          },
        })
