  ...
  <div v-if="movie">
    <div>
      <img
        ...
        class="gridimage animate__animated animate__jackInTheBox"
      />
      <div class="hoverbtn animate__animated animate__fadeInUp" style="">
        <button @click="goPostReview" class="w-btn w-btn-blue fun-btn">
        ...
        <span class="showlikeuser" style="color: white">{{
          like_users_count
        }}</span>
        <!-- <div v-if="like_users">
					<h6>좋아요 누른 사람</h6>
...
export default {
  ...
  data() {
    return {
      ...
      // movie: null,
      like_users_count: this.movie.cnt_like_users,
  ...
  computed: {
    isLike() {
      return this.like_users.some((user) => {
  ...
  methods: {
    goPostReview() {
      this.$router.push({
        name: "PostReview",
        params: { movie_id: this.movie.id, movie: this.movie },
      });
    },
    likeMovie() {
      axios({
    ...
    getMovie() {
      axios({
        ...
        .then((res) => {
          const data = res.data;
          // this.movie = res.data
          this.isLike = data.like_users.some((user) => {
            return user.id === this.user_id;
          });
